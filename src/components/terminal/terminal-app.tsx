...
export function TerminalApp() {
  ...
  const quotesQuery = useQuery({
    ...
    queryFn: async () => {
      const chunks: string[][] = [];
      for (let i = 0; i < watchSymbols.length; i += 40) {
        chunks.push(watchSymbols.slice(i, i + 40));
      }
      const results = await Promise.all(
        chunks.map((symbols) => fetchQuotes({ data: { symbols } })),
      );
      const quotes: Quote[] = [];
      const errors: string[] = [];
      for (const res of results) {
        if (res.ok) quotes.push(...res.quotes);
        else errors.push(res.error);
      }
      if (!quotes.length && errors.length) {
        return { ok: false as const, error: errors[0]!, quotes };
      }
  ...
  return (
    ...
    >
      <div className="terminal-shell flex h-[100svh] flex-col bg-bg text-fg">
        <HeaderBar />
        <SymbolSearch />
